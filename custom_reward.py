...
# 保留原来的函数签名作为备用（向后兼容）
def compute_score_legacy(response: str, reference: Dict[str, Any]) -> float:
    """
    原始的计算总体奖励分数函数（向后兼容）
    Args:
        response: 模型生成的回答文本
        reference: 包含参考答案和其他信息的字典
    Returns:
        float: 0-1之间的奖励分数
    """
    return compute_score(response, reference, "legacy")
...
if __name__ == "__main__":
    ...
    test_math_reference = {
        "answer": 14.0,
        "question": "如果12个苹果每个3元，顾客给了50元，应该找零多少？"
    }

    math_score = compute_score(test_math_response, test_math_reference, "openai/gsm8k")
    print(f"数学题测试分数: {math_score}")
    
    # 测试NER示例
    print("\n=== 测试NER任务 ===")
    test_ner_response = """
    <analysis>
    分析文本内容，用户询问的是关于Apple Macintosh电脑的描述。
    在文本中找到了"using a Mac"这个表述，其中"Mac"是Apple Macintosh电脑的简称。
    </analysis>
    <ner_result>["Mac"]</ner_result>
    """
    
    test_ner_ground_truth = ["Mac"]
    
    ner_score = compute_score(test_ner_response, test_ner_ground_truth, "ner_dapo")
    print(f"NER任务测试分数: {ner_score}")
